...
This project is a **minimal, thesis-ready scaffold** for evaluating an AI-driven test-generation agent
that uses a small(-ish) model (e.g. Google Gemini) to generate `pytest` tests for a Python module, and
then evaluates them using coverage, mutation testing (optional), and flakiness metrics.

## High-Level Flow

1. You provide a Python module, e.g. `src/utils/math_ops.py`.
2. The **agent** discovers functions in the module using `ast`.
3. For each function, it sends a prompt to the **Gemini API** (or a dummy local generator) asking for
   `pytest` tests.
...
This code is intentionally simple and well-commented so you can extend it for your thesis.
...
- Call the Gemini-based agent (or dummy generator)
